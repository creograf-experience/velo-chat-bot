...
const { leave } = Stage;
const allSubscribeInfo = new Scene('allSubscribeInfo');


allSubscribeInfo.enter(async (ctx: ContextMessageUpdate) => {
  const training = await Training.findOne({'_id':ctx.session.idSubscribeTraining});
...
allSubscribeInfo.hears('◀️ Назад', 
    updateUserTimestamp,
    asyncWrapper(async (ctx:ContextMessageUpdate) => await ctx.scene.enter('subscribeInfo'))
);



export default allSubscribeInfo;