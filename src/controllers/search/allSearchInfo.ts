...
const { leave } = Stage;
const allSearchInfo = new Scene('allSearchInfo');


allSearchInfo.enter(async (ctx: ContextMessageUpdate) => {
  const training = await Training.findOne({'_id':ctx.session.idSearchTraining});
